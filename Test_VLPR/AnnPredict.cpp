...
String AnnPredict::doPredict(Mat plate)
{
	...
	for (int i = 0; i < vec_ann_rects.size()&&count; i++,count--)
	{
...
//计算每一行像素的跳变数，如果小于指定最大阈值，认为该行是铆钉行，将该行像素值全部改为0，涂黑
bool AnnPredict::clearMaoDing(Mat &plate)
{
	int maxChangeCount = 12;//最大跳变次数阈值
	...
	printf("车牌字符高度百分比：%f", heightPercent);
