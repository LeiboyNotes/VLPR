...
int PlateLocate::verifySizes(RotatedRect rotateRect) {
	//容错率
	float error = 0.75f;
	//理想宽高比
	float aspect = float(136) / float(36);
	//真实宽高比
	float realAspect = float(rotateRect.size.width) / float(rotateRect.size.height);
	if(realAspect<1) realAspect = float(rotateRect.size.height) / float(rotateRect.size.width);
	//真实面积
	float area = rotateRect.size.height * rotateRect.size.width;
	//最小  最大面积  不符合的丢弃
	//给个大概就行   随时调整
	//尽量给大一些没关系   这还是初步筛选
	...
	//比例浮动   error认为也满足
	//最小宽高比
	float aspectMin = aspect - aspect * areaMin;
	//最大宽高比
